...
#ifdef REB_64
    // TOD 2013-09-25: something in the pool size appears to be assuming
    // the width is divisible by 32. The code above is too complicated to figure out for now
    // TODO: fix this so the size alignment is calculate properly (suspect it will be different for larger items)
    REBCNT poolWidth = Mem_Pools[pool_num].wide;
    REBCNT alignedSize = ALIGN(size, 32);
    // Some temporary variables for debugging purpose:
    //REBCNT seriesWidth = SERIES_WIDE(series);
    //REBCNT seriesRest = SERIES_REST(series);
    if (poolWidth != size) {
      if (poolWidth != alignedSize ) {
        ASSERT( FALSE, RP_FREE_NODE_SIZE);
      }
    }
#else // !REB_64, i.e. 32-bit architecture:
